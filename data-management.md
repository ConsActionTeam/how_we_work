# Conservation Action Team Data Management Plan

The Conservation Action Team strives for an open science policy that makes our science accessible and reproducible and that allows us to collaborate with our future selves and with our colleagues. To assist in achieving that aim, we use the following practices.
...
1. Each new project gets a GitHub repository in the [consactionteam](https://github.com/ConsActionTeam)
...
1. If the repo is included in a publication, we also archive it with a DOI on, for example, [Zenodo](https://www.zenodo.org) (see [here](https://github.com/consactionteam/consactionteam_methods/blob/master/cookbook.md#archiving-a-git-repo-with-zenodo-for-a-publication) for instructions).
2. We add a license to each git repo, e.g., CC BY-NC 4.0
3. We write collaborative manuscripts in the Conservation Action Team Google Drive
4. Presentations are typically made in Google Slides in the Conservation Action Team Google Drive (Presentations/) allowing lab members easy access to useful graphics
...
1. Raw data is stored in or linked from the GitHub repository associated with the project
   ...
   2. Larger files are stored in a data_largefiles/ directory (not tracked by Git by using the .gitignore file) in at least two places 
   3. If data from an external or public source is being used, it is stored in a data_dl/ directory (not tracked by Git). We clearly and unambiguously describe the data source in the data_dl/README.md file by providing links, version numbers, descriptions for access/download, or other details to ensure reproducibility. If you're worried the data won't be available in the future, follow one of the two previous bullet point approaches.
   4. Metadata is stored in the same directory as the raw data, typically in a README.md file that describes the data in each column, units, coordinate reference system (CRS, for GIS data), and other details needed to understand the file
   5. Processed or cleaned data is stored in a separate directory, e.g., output/ or similar, to differentiate from raw data
...
1. Raw data files too large for Github and Drive should be stored in a Dropbox/OneDrive/external hard drive, as appropriate.
